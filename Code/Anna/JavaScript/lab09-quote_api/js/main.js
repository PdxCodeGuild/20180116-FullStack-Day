...
let bt_another = document.querySelector('#bt_another');
...
function getFilterQuote(url, success) {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState === 1) {
            xhttp.setRequestHeader('Authorization', 'Token token="<sorry git robots, no token for you!>"');
        } else if (this.readyState === 4 && this.status === 200) {
            let data = JSON.parse(xhttp.responseText);
            success(data);
        } else if (this.readyState === 4 && this.status === 404) {
...
function makeNewCall() {
    ...
    getFilterQuote(url, displayMultiple);
}
