...
//SETTING UP INST initial values
let reverb = new Tone.Freeverb({
  "roomSize" : 0.5,
  "dampening" : 3000
}).toMaster();
// currently disabled
// delay value is a power of 2 of 
...
let synth = new Tone.MonoSynth({
			// "portamento" : 0,
			// "oscillator" : {
			// 	"type" : wav_ddl.options[wav_ddl.selectedIndex].value
			// },
			// "envelope" : {
			// 	"attack" : 0.01, 
   ...
   //    }, 
			// "filterEnvelope" : {
			// 	"attack" : 0.01,
			// 	"decay" : f_dec_sl.value,
			// 	"sustain" : f_sus_sl.value,
			// 	"release" : f_rel_sl.value,
			// 	"baseFrequency" : f_frq_sl.value,
			// 	"octaves" : f_q_sl.value //referred to as q
   //      // "rolloff" : -24 //
			// }
		}).connect(delay); 

let stop = false;
let note_array = ['A','B','C','D','E','F','G'];//test array
...
play_bt.onclick = function() {
  ...
  let starting_note = `${note_array[Math.floor(Math.random() * (note_array.length - 1))]}2`; //hardcoded to 2oct currently
...
function new_sequence(length) {
  ...
  function update_values() {
    ...
    synth.filterEnvelope.Q = f_q_sl.value;
    synth.filterEnvelope.gain = f_gain_sl.value;

    console.log(synth.filterEnvelope.Q);
      //SYNTH trigger note on interval
     // octave shift value is multiplying freq by pow of 2   
    synth.triggerAttackRelease((sequence_1[sequence_position] * Math.pow(2,s_oct_sl.value)), note_length);
    cursor_position += 1;
